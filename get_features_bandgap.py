...
def create_bandgap_feature_dataset(data):
    """
    Create feature-engineered dataset using MatPipe for bandgap prediction.
    """
    ...
    print("\nCreating bandgap feature dataset...")
    df_bandgap = df_features.copy()
    df_bandgap['target'] = final_bandgaps
    
    # Save to CSV
    output_file = '2d_materials_bandgap_features.csv'
    df_bandgap.to_csv(output_file, index=False)
    ...
    small_gap = sum((final_bandgaps_array > 0.0) & (final_bandgaps_array <= 0.5))
    medium_gap = sum((final_bandgaps_array > 0.5) & (final_bandgaps_array <= 2.0))
    large_gap = sum(final_bandgaps_array > 2.0)
    
    print(f"\nBandgap distribution:")
    print(f"  - Small gap (0.0 < gap ≤ 0.5 eV): {small_gap} ({small_gap/len(final_bandgaps)*100:.1f}%)")
    print(f"  - Medium gap (0.5 < gap ≤ 2.0 eV): {medium_gap} ({medium_gap/len(final_bandgaps)*100:.1f}%)")
    print(f"  - Large gap (gap > 2.0 eV): {large_gap} ({large_gap/len(final_bandgaps)*100:.1f}%)")
...
def main():
    ...
    # Extract structures and bandgap data
    ...
    # Create feature dataset for bandgap prediction
    df_bandgap = create_bandgap_feature_dataset(data)
